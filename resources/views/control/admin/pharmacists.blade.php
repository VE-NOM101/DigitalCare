                ...
                <div class="card-body">
                    ...
                    <div class="tab-content pt-2" id="myTabjustifiedContent">
                                ...
                                <tbody>
                                    @foreach ($getRecord as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ url('_admin/delete_pharmacists/' . $item->id) }}"
                        ...
                        <div class="tab-pane fade" id="profile-justified" role="tabpanel" aria-labelledby="profile-tab">
                            <form action="{{ url('/_admin/add_pharmacists') }}" method="POST">
                                @csrf
                                <div class="row mb-3">
                                    <label class="col-sm-2 col-form-label">ID<-->Email</label>
                                    <div class="col-sm-10">
                                        <select name="user_id" class="form-select" aria-label="Default select example">
                                            <option value="{{ $getPharmacist->first()->id}}">
                                                {{ $getPharmacist->first()->id .'<--> '.$getPharmacist->first()->email}}
                                            </option>
                                            @foreach ($getPharmacist as $item)
                                                @if ($item->id != $getPharmacist->first()->id)
